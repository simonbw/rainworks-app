...
const ReportsText = ({ rainwork, ...otherProps }) => (
  <ReportsConsumer>
    {({ getReport }) => {
      ...
      return (
        <View {...otherProps}>
          <Text style={styles.foundItCount}>{getFoundItCountText(Boolean(foundItReport), rainwork['found_it_count'])}</Text>
          
          {foundItReport && (
            <Text style={styles.foundItText}>
              You found this rainwork on {moment(foundItReport['created_at']).format(COMMON_DATE_FORMAT)}.
            </Text>
          )}
          
          {missingReport && (
            <Text style={styles.reportText}>
              You reported this rainwork missing
              on {moment(missingReport['created_at']).format(COMMON_DATE_FORMAT)}.
            </Text>
          )}
          
          {fadedReport && (
            <Text style={styles.reportText}>
              You reported this rainwork faded
              on {moment(fadedReport['created_at']).format(COMMON_DATE_FORMAT)}.
            </Text>
          )}
          
          {inappropriateReport && (
            <Text style={styles.reportText}>
              You reported this rainwork as inappropriate
              on {moment(inappropriateReport['created_at']).format(COMMON_DATE_FORMAT)}.
            </Text>
          )}
        </View>
      );
